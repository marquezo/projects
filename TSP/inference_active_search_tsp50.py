...
tsp50_dataset = pickle.load(open("test_data_set_20", "rb"))
...
for batch_id, sample_batch in enumerate(tsp50_loader):
        ...
        if batch_id < 10:
                ...
                soln_sampling, min_tour_len = active_search(sample_batch, tsp50, 12800, 128, lr=1e-5)
                tour_lengths.append(min_tour_len)
                print ("batch id {}, result: {}".format(batch_id, min_tour_len))

# file = open('results_active_search_tsp50_128', 'wb')
# pickle.dump(tour_lengths, file)
# file.close()

print ("Average length of tour: ", np.mean(tour_lengths))
